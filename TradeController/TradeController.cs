...
namespace MBR
{
    public class TradeController
    {
        ...
        public TradeController Register(Robot robot)
        {
            ...
            robot.Bars.BarOpened += this.OnBar;
            ...
            this.Robot = robot; //register fucking robot
        ...
        public bool AddPendingOrder(OperationType operationType, double entryPrice, double stopLostPips, double takeProfitPips, double lots, List<Operation.Rules.Rule> rules = null)
        {
            ...
            {
                TradeResult result = Robot.PlaceStopOrder(TradeType.Sell, symbol.Name, volume, entryPrice, "Auto operation", stopLostPips, takeProfitPips);
                if (!result.IsSuccessful) return false;
                cAlgo.API.PendingOrder p = result.PendingOrder;

                op = new Operation.PendingOrder(p.Id, OperationType.SellStop, (double)p.StopLoss, (double)p.TakeProfit, this.symbol, entryPrice);
            }
            //sell limit
            if (operationType == OperationType.SellLimit)
            {
                TradeResult result = Robot.PlaceLimitOrder(TradeType.Sell, symbol.Name, volume, entryPrice, "Auto operation", stopLostPips, takeProfitPips);
                if (!result.IsSuccessful) return false;
                cAlgo.API.PendingOrder p = result.PendingOrder;

                op = new Operation.PendingOrder(p.Id, OperationType.SellLimit, (double)p.StopLoss, (double)p.TakeProfit, this.symbol, entryPrice);
            }
            //buy limit
            if (operationType == OperationType.BuyLimit)
            {
                ...
                op = new Operation.PendingOrder(p.Id, OperationType.BuyLimit, (double)p.StopLoss, (double)p.TakeProfit, this.symbol, entryPrice);
            }
            //buy stop
            if (operationType == OperationType.BuyStop)
            {
                ...
                op = new Operation.PendingOrder(p.Id, OperationType.BuyStop, (double)p.StopLoss, (double)p.TakeProfit, this.symbol, entryPrice);
            }
        ...
        public bool AddPosition(OperationType operationType, double stopLostPips, double takeProfitPips, double lots, List<Operation.Rules.Rule> rules = null)
        {
            ...
            {
                TradeResult result = Robot.ExecuteMarketOrder(TradeType.Sell, symbol.Name, volume, "Auto operation", stopLostPips, takeProfitPips);
                if (!result.IsSuccessful) return false;
                cAlgo.API.Position p = result.Position;

                op = new Operation.Position(p.Id, OperationType.Sell, (double)p.StopLoss, p.TakeProfit, this.symbol, p.EntryPrice, DateTime.Now);
            }

            if (operationType == OperationType.Buy)
            {
                TradeResult result = Robot.ExecuteMarketOrder(TradeType.Buy, symbol.Name, volume, "Auto operation", stopLostPips, takeProfitPips);
                if (!result.IsSuccessful) return false;
                cAlgo.API.Position p = result.Position;

                op = new Operation.Position(p.Id, OperationType.Buy, (double)p.StopLoss, p.TakeProfit, this.symbol, p.EntryPrice, DateTime.Now);
            }
        ...
        public void OrderFilled(PendingOrderFilledEventArgs obj)
        {
            ...
            if (po != null)
            {
                ...
                Operation.Position p = new Operation.Position(obj.Position.Id, type, Convert.ToDouble(obj.Position.StopLoss), obj.Position.TakeProfit, this.symbol, obj.Position.EntryPrice, obj.Position.EntryTime);
                Operations.Add(p);
        ...
        private void OnBar(BarOpenedEventArgs obj)
        {
            Parallel.ForEach<OperationBase>(Operations, operation =>
            {
